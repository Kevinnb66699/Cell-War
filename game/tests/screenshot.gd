...
##                       **比的是 `get_global_name()`，所以要写那个实例真正的类名**（基类名找不到）。
##                       纯鼠标的 Control 按钮就靠这一条
## 交互态（行动栏、目标选择、悬浮框）不点几下根本到不了，而这些恰恰是最需要
## 和设计稿比对的画面。例：`"420,240@1.2;640,500@4.0"`
##
## ── 新手教程 v2 的步骤（**2026-09-19 · S12 按新壳重写**；老教程那套 `CWGuide*` 整份没了）──
## 能驱动的四件（`call:` 的类名都是**实例的类名**，别写基类 `CWTutorView`）：
##   `call:CWTutorViewBubble:advance`  按「继续」翻过一条 `say`（皮 A，缺省。占位皮写
##                                     `CWTutorViewPlain:advance`，换皮开关是 `CWSettings.tutor_skin`）
##   `call:CWTutorChrome:toggle_menu`  开 / 收左上角「目录」面板（S6）
##   `call:CWTutorChrome:press_switch` 按「切换种类」（第五关 Step2，S5）
##   `key:ui_cancel` / `key:ui_up` / `key:ui_accept`   主菜单与暂停菜单那一路
## 从主菜单进教程：三下 `ui_up`（循环选择，从「开始对局」倒着数到「新手引导」）+ 一下 `ui_accept`。
## 只要这一下（全通关过的存档也一样：「教程对手癌种」四选一 2026-09-25 起删了）。
## 棋盘落子与行动栏按钮走 `x,y@秒数` 合成点击（2026-09-19 实测点得动），但**一步一等**：
## 一句台词一拍 advance（`say` 的每一行各一拍，提示行 `hint` 再一拍）、行动栏建出来要 ≈1 秒、
## 迁移一步隔 0.8 秒、攻击结算等 3~4 秒。
## ⚠ **教程局的棋盘格→屏幕坐标不是固定的**：每关的 `ui.camera` 自己算倍率与看点
##（`CWView.tutor_framing`，活跃格少的关会推到 4 倍），所以坐标要**先截一帧量出来**，
## 别照搬正式局那套（`_look(1.0)` 下 `(0,-1)` = (365,240)、每 +1 q 加 46.2px）。
## ⚠ 直接开到某一关要改 `user://guide_progress.cfg` 的 `done`（**结尾那个换行不能少**，
## 少了 `ConfigFile.load` 直接失败、一声不吭地从第一关开）——那是玩家的真进度，拍完还原。
