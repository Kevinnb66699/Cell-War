## cw_case_loader.gd —— L0 用例的 `world` 段 ⇄ CWGame（测试迁移规格 A-2 / C-1 步 8；键表口径 = 规格 §0.6.1）
##
## 三处用它：l0_runner.gd（跑探针与契约步）、l0_pre_dump.gd（闸二 2b）、tests/rec/（录制代理落 pre/post）。
## 显式键表：用例里出现表外的键 = 硬错（闸 2c）。**仓库里只许两份键表**：这一份与 L0/CaseModel.cs，逐键相同。
##
## 四条口径（§0.6.1，改之前先读）：
