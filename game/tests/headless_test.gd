...
func t_tutorial() -> void:
	...
	m._guide._step = 1
	m._guide._render()
	check(m._guide._hint.text == CWGuideBridge.STEP_HINTS["place"]["hint"]
		and m._guide._hint_tail.text == CWGuide.OFFER_TAIL % "继续",
		"翻到「第一步：落子」：行动框是落子提示，代做尾巴贴到主按钮上方（%s | %s）"
	...
	check(m._guide._hint.text == CWGuideBridge.STEP_HINTS["end"]["hint"]
		and m._guide._hint_tail.text == CWGuide.OFFER_TAIL % "下一章",
		"翻到「别忘了结束回合」：提示跟着换成结束回合那句 + 代做尾巴（%s | %s）"
		% [m._guide._hint.text, m._guide._hint_tail.text])
	m._guide._step = 2
	m._guide._render()
	...
	m._guide._render()
	check(m._guide._btn.text == "下一章" and m._guide._hint_tail.text == CWGuide.OFFER_TAIL % "下一章",
		"关卡最后一步：尾巴写的是「下一章」（%s | %s）" % [m._guide._btn.text, m._guide._hint_tail.text])
