...
const PROTO := 2
## 与 C# `CanonCodec.TuneToCanon` 同一份键（21 个）；两边各自从自己的旋钮对象读
const TUNE_KEYS := [
	"cancer_move_cancerous", "cancer_move_healthy", "sclc_move_healthy", "pseudopod_cost",
	"mucus_move_surcharge", "metastasis_cost", "metastasis_max_per_round", "immune_respawn_delay", "macro_heal_purify", "attack_max_per_turn", "anaerobic_solid_bonus", "anaerobic_floor",
