...
func _audit() -> void:
	var sets: Array = [["第一关", _a1()], ["第二关", _a2()], ["第三关", _a3()],
		["第四关", _full()], ["第五关", _full()], ["第六关初始", _full()],
		["第六关第23步", _full()]]
	var prev: Array = []
...
## 癌组织连通块两两至少隔 3 格（Kevin 09-19 第三轮 ②：块之间别挨着）。
func _check_blocks(tag: String, blocks: Dictionary) -> void:
	if blocks.is_empty():
		return
	...
	for i in names.size():
		for j in range(i + 1, names.size()):
			for a: Vector2i in blocks[names[i]]:
				for b: Vector2i in blocks[names[j]]:
					var d: int = CWData.hex_dist(a, b)
					if d < best:
						best = d
	...
	var sizes: Array = []
	for n in names:
		sizes.append("%s %d" % [n, blocks[n].size()])
	print("[癌块] %s：%s；块间最近 %d 格（%s）" % [tag, "　".join(sizes), best, pair])
	if best < 3:
		push_error("[癌块] %s 块间贴太近：%d 格（%s）" % [tag, best, pair])
...
func _build(cfg: Dictionary) -> void:
	...
	_check_blocks(String(cfg.get("tag", "?")), cfg.get("blocks", {}))
...
## 第三关右端那个「外突」的癌组织连通块（v4：把 r=0 排右段 (3,0)(4,0)(5,0) 并进来）。
## 9 格：中排 4 格 + 柄 (5,-2)(5,-3) 出横带 + 下排 3 格。全关只有这一块。
...
## 第四关四个癌组织连通块，**每块 10 格**（Kevin 09-19 第三轮 ②），块与块两两至少隔 3 格。
## A 是第三关那块长出来的 —— 但 (3,-1)(4,-1) 被玩家在第三关净化掉了，不再是癌组织。
func _l4_blocks() -> Dictionary:
	return {
		"A": [Vector2i(5, -1), Vector2i(6, -1), Vector2i(6, -2), Vector2i(6, -3),
			Vector2i(5, -2), Vector2i(5, -3),
			Vector2i(3, 0), Vector2i(4, 0), Vector2i(5, 0), Vector2i(6, 0)],
		"B": [Vector2i(0, -3), Vector2i(1, -3), Vector2i(2, -3),
			Vector2i(0, -4), Vector2i(1, -4), Vector2i(2, -4),
			Vector2i(0, -5), Vector2i(1, -5), Vector2i(2, -5), Vector2i(3, -5)],
		"C": [Vector2i(-4, 1), Vector2i(-3, 1), Vector2i(-2, 1),
			Vector2i(-5, 2), Vector2i(-4, 2), Vector2i(-3, 2), Vector2i(-2, 2),
			Vector2i(-5, 3), Vector2i(-4, 3), Vector2i(-3, 3)],
		"D": [Vector2i(0, 3), Vector2i(1, 3), Vector2i(2, 3),
			Vector2i(-1, 4), Vector2i(0, 4), Vector2i(1, 4), Vector2i(2, 4),
			Vector2i(-1, 5), Vector2i(0, 5), Vector2i(1, 5)],
	}

## 第五关 Step2：四块各长两格 → **12 格**（PRD:361「使已有癌组织连通块扩大」），
## 其中三格转固化（PRD:363）。块间仍两两至少隔 3 格。
func _l5_blocks() -> Dictionary:
	var b := _l4_blocks()
	b["A"] = b["A"] + [Vector2i(4, -2), Vector2i(5, 1)]
	b["B"] = b["B"] + [Vector2i(3, -4), Vector2i(1, -6)]
	b["C"] = b["C"] + [Vector2i(-5, 1), Vector2i(-6, 3)]
	b["D"] = b["D"] + [Vector2i(-2, 5), Vector2i(0, 6)]
	return b
...
## 第五关留到第六关的三格固化（都避开第六关围圈的那一环）。
func _l5_solid() -> Array:
	return [Vector2i(6, -1), Vector2i(1, -5), Vector2i(2, 3)]


## 第一关（PRD:91-137）。Kevin：第一、二关不动。
func _f1() -> void:
	_build({
		"tag": "第一关",
		"title": "第一关 免疫 —— 2 格（第一、二关不动）",
		"ask": [
			"圈①　两格的位置：默认 (-5,-1) 起、(-4,-1) 落（深青）。摆在 r=-1 排的**最左端**是整套几何的地基 —— 这一排 q 从 -5 到 6 共 12 格、跨度 11。",
			"圈②　玩家起点要不要和开场动画里细胞跌落的落点对上？（开场动画保留不动。）",
			"长期账：这两格到第六关还在原处 —— (-5,-1) 最后站着那只要杀死你的 T 细胞、(-4,-1) 是你最后一次【转移】的落点，首尾在同两格上收口。",
...
## 第三关（PRD:223-283）。v4：r=0 排右段 (3,0)(4,0)(5,0) 并进癌块 ⇒ 能量 6.1 → **6.6**。
func _f3() -> void:
	var active := _a3()
	var block := _l3_block()
	var marks := {}
	## 两条并列最省路的落点（都 3.0）：直线走停 (4,-1)，绕 r=-2 排停 (5,-2)
	marks[Vector2i(4, -1)] = _board.MARK_PLAN
	marks[Vector2i(5, -2)] = _board.MARK_PLAN
	_paint(marks, block, _board.MARK_ATTACK)
	_paint(marks, _minus(active, _a2()), MARK_NEW)
	_build({
		"tag": "第三关",
		"title": "第三关 ATP —— 37 格；癌块 9 格（含 r=0 排右段），能量 6.1 → 6.6",
		"ask": [
			"圈①　横带 = r=-2 / r=-1 / r=0 三整排（PRD:227）。横带之外只有一格：(5,-3)。",
			"圈②　**v4 改动**：(3,0)(4,0)(5,0) 也变成癌组织，并进右端那个外突连通块 ⇒ 块从 6 格长到 **9 格**：中排 (3,-1)(4,-1)(5,-1)(6,-1) + 柄 (5,-2)(5,-3) 出横带 + 下排 (3,0)(4,0)(5,0)。3.0 能量的癌细胞仍站 (5,-1)。",
			"圈③　**能量要重算**：癌细胞一环里只剩 (6,-2) 一格健康，而它只能穿过癌组织到达（4.0）。最省 = **3.0**，两条并列（深青两个落点）：① 直线 (1,-1)(2,-1) 健康 + (3,-1)(4,-1) 癌 = 0.5×2+1.0×2；② 绕 r=-2 排 (1,-2)(2,-2)(3,-2)(4,-2) 健康 + (5,-2) 癌 = 0.5×4+1.0。**能量 6.6** = 3.0 + 攻击三次 3.0 + 失效自损 0.5 + 0.1。",
			"圈④　劝重置阈值仍是 **35**（十分能量）：走 3.0 那两条剩 36 不劝；次优的 (6,-2) / (4,0) 都是 3.5，剩 31 当场劝。最省路从「唯一」变成「两条并列」，要不要微调让它唯一？",
			TIP_COLOR,
		],
		"active": active,
		"cancer": block,
...
## 第四关（PRD:287-328）。整盘 127 格；v4：四块各 10 格。
func _f4() -> void:
	var blocks := _l4_blocks()
	var cancer := _flat(blocks)
	var marks := {}
	_paint(marks, cancer, _board.MARK_ATTACK)
	_build({
		"tag": "第四关",
		...
		"blocks": blocks,
		"title": "第四关 抗原记忆 —— 整盘 127 格；四块各 10 格（共 40 格癌组织）",
		"ask": [
			"圈①　「向周围延伸」= 把半径 6 的 127 格一次全揭开，浮现按环错峰。第五关 / 间章 / 第六关都保持这个大小。",
			"圈②　**v4 改动**：四个连通块各放大到 **10 格**（橙）。A 右（第三关那块长出来的；(3,-1)(4,-1) 已被玩家净化，不再是癌组织）/ B 上 / C 左下 / D 下。**块与块两两至少隔 3 格**。",
			"圈③　癌细胞 5 只：(5,-1) 5.0 / (3,0) 2.0 / (1,-4) 4.0 / (-3,2) 3.0 / (1,4) 3.0，两两至少隔 2 格。记忆账 = 净化 40 格 + 打掉 17.0 能量 = 57，升 III 只需 +10（起手 II 级 20 记忆）—— 富余很大，玩家想怎么打都行。要不要减块 / 减细胞？",
			"圈④　玩家起手 (4,-1)：第三关最省路走完就站在这格（它被玩家净化成健康组织，四周被 A 块围着但没围满，不会触发【微环境压迫】—— 何况第一～五关永远进不到 E 阶段）。",
			TIP_COLOR,
		],
		"active": _full(),
		"cancer": cancer,
		...
		"cells": [
			...
			{ "kind": "osteo", "at": Vector2i(3, 0), "seat": 2, "name": "骨肉瘤", "e": "2.0" },
			{ "kind": "osteo", "at": Vector2i(1, -4), "seat": 3, "name": "骨肉瘤", "e": "4.0" },
			{ "kind": "osteo", "at": Vector2i(-3, 2), "seat": 4, "name": "骨肉瘤", "e": "3.0" },
			{ "kind": "osteo", "at": Vector2i(1, 4), "seat": 5, "name": "骨肉瘤", "e": "3.0" },
		],
	})


## 第五关 Step2（PRD:329-383）。整盘不变；四块各 12 格 + 三格固化 + 各类免疫各一。
func _f5() -> void:
	var blocks := _l5_blocks()
	var solid := _l5_solid()
	var cancer := _minus(_flat(blocks), solid)
	var marks := {}
	...
	_build({
		...
		"anchor": FULL_ANCHOR, "avail": FULL_AVAIL,
		"blocks": blocks,
		"title": "第五关 分化 Step2 —— 整盘不变；四块各 12 格 + 固化 3 格 + 9 席",
		"ask": [
			"圈①　四块各长两格（10 → **12 格**，PRD:361「使已有癌组织连通块扩大」），块与块仍两两至少隔 3 格。",
			"圈②　固化癌组织三格（深青）：(6,-1) / (1,-5) / (2,3)，写 state:solid 不是 solid 计数。它们到第六关还在 —— 是「复活窗口必须有一格不被免疫占据的固化格」那条硬约束的余量，而且都**避开了第六关围圈的那一环**。",
			"圈③　「各种类免疫各一」：B (1,-2) / 巨噬 (6,-4) / 树突 (0,2) / **T (-5,-1)**，加玩家 (4,-1) 共 5 只，两两至少隔 3 格。巨噬摆在离 (6,-2) 最近的位置 —— 间章「最近一个免疫细胞靠过来攻击」点名的就是它；T 摆在走廊最远端，就是第六关那只。",
			"圈④　癌细胞 4 只（5.0 / 4.0 / 3.0 / 2.0），两两至少隔 5 格。9 席到顶（玩家 + 4 免疫 + 4 癌）—— 第四关那 5 只到这一关收成 4 只，就是为了留出四个免疫席。",
			TIP_COLOR,
		],
		"active": _full(),
		"cancer": cancer,
		...
		"cells": [
			...
			{ "kind": "osteo", "at": Vector2i(1, 4), "seat": 8, "name": "骨肉瘤", "e": "2.0" },
		],
	})


## 第六关开局（PRD:419-465；09-19 03:41 版把「第七关」改名「第六关」）。
## Kevin：**第六关盘面暂时不动** —— 这一帧与 v3 逐格相同，只改了标题里的关号。
func _f7a() -> void:
	...
	_build({
		"tag": "第六关初始",
		"anchor": FULL_ANCHOR, "avail": FULL_AVAIL,
		"title": "第六关 初始 —— 整盘不变（Kevin：第六关盘面暂不动，与 v3 逐格相同）",
		"ask": [
			"圈①　围一圈围的是间章里靠过来又被击退 1 格的那只巨噬 (4,-2)，一环 6 格（橙）。玩家从固化格 (6,-2) 出发 6 步：(5,-2)(5,-3)(4,-3)(3,-2)(3,-1)(4,-1)，收在 (4,-1)（深青）—— 到 T (-5,-1) 正好 9 格。",
			"圈②　几何锁（四个深青）：距 9 →「进 1 退 2」两轮 → 落 (6,-1) 距 11 = 5×2+1 → 两次【转移】(1,-1) → (-4,-1) 正好邻接 T。全程走 r=-1 排；(6,-1) 是这一排的最右端，击退顶死在盘边、一格不多。",
			"圈③　免疫四只（T (-5,-1) / 巨噬 (4,-2) / B (1,-2) / 树突 (0,2)）两两至少隔 2 格。T 站的正是第一关玩家出生那格，也是棋盘真外环（s=6）。",
			"圈④　**要 Kevin 拍板**：PRD 03:41 版间章分镜 2 新写了「地图以免疫细胞为中心向四周延伸，补齐缺失格子使其处于一个完整棋盘的中央」。第四关已经是整盘 ⇒ 没有缺失格子可补。两条路（提案文档 §6.2 单开一节）：A 这一条退化成纯镜头动作；B 间章分镜 6 那次换局把世界**重心平移到玩家**。**选 B 的话本帧整套几何作废** —— 玩家居中时到任何格最远只有 6，距离 9 / 11 根本不存在（重摆版见文档表）。",
			TIP_COLOR,
		],
		"active": _full(),
		"solid": solid,
...
## 第六关第 23 步（PRD:503）。与 v3 逐格相同，只改标题里的关号。
func _f7b() -> void:
	...
	_build({
		"tag": "第六关第23步",
		"anchor": FULL_ANCHOR, "avail": FULL_AVAIL,
		"title": "第六关 第 23 步 —— 边缘再生 T / B 之后（与 v3 逐格相同）",
		"ask": [
			"圈①　「地图边缘再次生成若干 T 细胞和 B 细胞」= 棋盘真正的边缘（半径 6 外环）：T (6,-4) / B (6,-6) / T (4,-6)（橙），两两至少隔 2 格，围着复活点 (6,-2)。",
			"圈②　复活站哪格固化（深青 (6,-2)）？默认回**出生那格**。代价：【黏液破裂】发生在 (-4,-1)，最后一幕整个搬回右边。另一条路是在 T 那一头另设一格固化就地收尾。",
...
func _f8() -> void:
	...
	_build({
		...
		"ask": [
			"长　%s　= 127 格。**第五关 / 间章 / 第六关都不再加格** —— 第四关一次揭到正常棋盘大小之后就不动了。" % "　".join(counts),
			"长　颜色：青 1 关（2 格）　橙 2 关（+5）　绿 3 关（+30：3 格宽整横带 + 右端外突那一格 (5,-3)）　紫 4 关（+90：整盘）。每一关的活跃格都是前一关的严格超集，已露出的格子一格没挪过位置。",
			"长　走向：第一～三关沿 r=-1 排从最左端 (-5,-1) 向右长（第三关起宽到 3 整排），第四关一次揭完。第六关那条「距离 9 + 击退 2」的直线，就是第一～三关走出来的那条走廊。",
			"圈　PRD 03:41 版间章分镜 2 要求「地图以免疫细胞为中心补齐成完整棋盘的中央」。这张图从第四关起就已经是完整棋盘，但**玩家不在盘心**。要不要在间章把世界重心平移到玩家？见提案文档 §6.2。",
			TIP_COLOR,
		],
		"active": _full(),
		"marks": marks,
