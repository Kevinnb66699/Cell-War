...
const STEP_HINTS := {
	...
	## 下面三个 2026-09-13 补，给剧本重写用（后半程一直是「只说不练」）。
	## 它们都**不进 AUTO_ACTS**：净化和攻击一样是两段式选格，
	## 分化还要选方向、打牌还要选目标 —— 代做等于替玩家做了决定（同落子那条拍板）。
	"purify": { "act": "move", "hint": "迁到一格癌组织上，走进去就地净化" },
	"differentiate": { "act": "differentiate", "hint": "点底部「分化」，挑一个方向" },
	"play": { "act": "", "hint": "在左下角手牌里双击一张打出去" },
}
