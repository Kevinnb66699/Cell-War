...
## 骰子**整层压在棋盘内容之上**（Kevin 2026-09-13 issue #36：「减伤特效应当在骰子图层之下」）。
## 症状：囊性护甲那两枚小盾绕着**前排**那只癌细胞转，按画家算法它们的 z（那一格 +3）
## 比**后排**格子上的骰子大，于是盖住了点数。
## 为什么不能再按 `tile_z` 排：那一套排的是「谁站在谁前面」，而骰子演的是**此刻的判定** ——
## 它要盖住的是**别人格子**上的东西，这在「按排比大小」的体系里根本表达不了。
## 取一个比任何一格都高、又低于全局演出层的常数（最大的一格 z 也就几百）。
const Z_DICE_TOP := 2048
...
##
## 第一版取癌组织贴图那块红提亮一档（#E05A6E）、混合比例照旧 0x6E —— 叠完 #C55163，
## 与**没高亮的**癌格 #B04A5A 色距只有 0.09，Kevin 2026-09-19：「癌性组织上方的滤镜太淡了，
## 稍微加深一点」。同色只抬比例没用（抬到 0.55 也才 0.12：色标本来就和底色是一家），
## 得把色标从底色上挪开：艳红 #FF3D5C、比例 0x8C ≈ 0.55，叠完 #DB435B，与未高亮癌格色距 0.17、
## 与健康候选格 0.75、与攻击橙叠色 0.21。候选帧见 `截图_2026-09-19_issue47_加深/`（五档里选的乙）。
const MARK_MOVE_SICK := Color("ff3d5c8c")
...
const TURN_ARROW_THICK := 2                        ## 箭的线粗几 px（Kevin 2026-09-13：「把箭头加厚」）
const TURN_ARROW_PERIOD := 0.5                     ## 每拍多久：低位、高位各停半秒


## 箭的像素（相对箭尖，y 向上为负）：一个 V，线**粗 TURN_ARROW_THICK 像素**
## （Kevin 2026-09-13：「把箭头加厚」—— 1 px 的细线在棋盘上太不起眼）。
## 加粗的方向是**往上叠**：每多一层就整条往上挪一格，于是箭尖始终是最低那一颗，
## `turn_tip_dy` 那套「箭尖离胞体几行」的算法一个字都不用改。**纯函数**
static func turn_arrow_pixels() -> Array:
	var out: Array = []
	for k in TURN_ARROW_THICK:
		out.append(Vector2i(0, -k))
		for i in range(1, 4):
			out.append(Vector2i(-i, -i - k))
			out.append(Vector2i(i, -i - k))
	return out
