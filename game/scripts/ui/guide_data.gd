...
static func _chapter_intro() -> Array:
	return [
		...
		{ "t": "这一局的目标", "flag": "", "b": [
			...
			"癌方要把癌组织铺开，让加权占地达到 %d。" % CWData.CANCER_WIN_WEIGHTED,
		] },
		{ "t": "认识棋盘", "flag": "board", "b": [
		...
		{ "t": "三种特殊组织", "flag": "special", "b": [
			"代谢核心：站上去会收到存储的能量，健康时每 2 回合产出 1.0。",
			"骨髓：站上去会获得卡牌，是免疫复活的锚点。",
			"血管：两端可让癌细胞【血行转移】。",
		] },
		{ "t": "看完这页就动手", "flag": "", "b": [
	...
	return [
		...
		{ "t": "能量就是生命", "flag": "energy", "b": [
			"右侧竖条显示你当前的能量。开局免疫 %s、癌方 %s。" % [
				CWData.fmt(CWData.INIT_ENERGY), CWData.fmt(CWData.INIT_ENERGY_CANCER)],
		...
		{ "t": "净化与记忆", "flag": "purify", "b": [
			...
			"每净化一格 +1 抗原记忆；记忆积累到 16 升 III 级、31 升 X 级。",
		] },
...
static func _chapter_attack() -> Array:
	return [
		...
		{ "t": "看懂骰子", "flag": "d6", "b": [
			"1-2 失败：免疫被弹回原格，并损失 %s 能量。" % CWData.fmt(CWData.COUNTER_DMG_ON_FAIL),
			"3-5 成功：目标失去 %s 能量。" % CWData.fmt(CWData.ATTACK_DMG_SUCCESS),
			"6 大成功：目标失去 %s 能量。" % CWData.fmt(CWData.ATTACK_DMG_CRIT),
		] },
		{ "t": "攻击次数", "flag": "attack_limit", "b": [
			"每个免疫细胞每行动回合最多攻击 %d 次。" % CWData.ATTACK_MAX_PER_TURN,
...
static func _chapter_cards() -> Array:
	return [
		...
		{ "t": "手牌上限", "flag": "hand_limit", "b": [
			...
			"骨髓每 3 个世界回合还会发一张卡，别忘了去收。",
		] },
		{ "t": "分化预览", "flag": "differentiate", "b": [
			"免疫等级升到 III 后可以免费分化（B/T/巨噬/树突）。",
			"每种分化全阵营限一个，尽量配齐阵容。",
		] },
	]
...
static func _chapter_world() -> Array:
	return [
		{ "t": "一个世界回合", "flag": "round", "b": [
			"S 阶段：世界事件、特殊组织产出、血管传送，然后复活。",
			"玩家回合：按行动顺序轮流行动。",
			"E 阶段：癌方结算收入、微环境压迫、增生、固化，最后判胜负。",
		] },
		{ "t": "癌方如何成长", "flag": "cancer_grow", "b": [
			"癌细胞每走进健康组织就【定殖】一格（花 %s 能量）。" % CWData.fmt(CWData.CANCER_MOVE_HEALTHY),
		...
		{ "t": "世界事件", "flag": "world_event", "b": [
			"第 3、6、10、15、20、25、30 世界回合会触发世界事件。",
...
static func _chapter_cells() -> Array:
	return [
		...
		{ "t": "分化阵容 ①", "flag": "differentiate", "b": [
			...
			"两者都要免疫等级 III 解锁，全阵营各限一个。",
		] },
		{ "t": "分化阵容 ②", "flag": "", "b": [
			"巨噬细胞：每次净化回 %s 能量，跟着大部队越打越富。" % CWData.fmt(CWData.MACRO_HEAL_PURIFY),
			"树突细胞：花 %s 能量建趋化源，免疫靠近免 30%%、癌方远离加 40%%。" % CWData.fmt(CWData.CHEMO_COST),
		...
		{ "t": "癌方 ②", "flag": "", "b": [
			"骨肉瘤：固化成型更快，站在固化组织上受伤只剩 %d%%。" % CWData.OSTEO_BARRIER_PERCENT,
			"小细胞肺癌：移入健康格只花 %s，还能向某方向跃 5 格。" % CWData.fmt(CWData.SCLC_MOVE_HEALTHY),
			"速度与滚雪球兼备，别让它把地盘滚大。",
		] },
		{ "t": "图鉴速查", "flag": "", "b": [
