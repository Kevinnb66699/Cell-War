## erosion_fx.gd —— 癌把一格健康组织吞掉的两帧过场（【E-侵蚀】【E-增生】【定殖】共用）
##
## 最早只为【E-侵蚀】接的（文件名、桥上的 show_erosion 都沿用）；2026-09-06 起【增生】【定殖】也走它——
## 三者对玩家是同一件事「这一格变癌了」，差别只在引擎侧怎么定「癌从哪一侧来」：
## 侵蚀 / 增生取 DIRS 顺序第一个癌性邻居（CWWorld._erosion_dir），定殖取细胞的来路（CWData.dir_toward）。
##
