...
const ITEMS := [
	...
	## 「新手引导」2026-09-14 起暂时停用；2026-09-19 Kevin 开过一次（「把新手教程启用」，热更 202609182342），
	## 同日又灰回去：Kevin「把之前教程的 UI 等设计全部删掉，基于剧本从 0 构建一个全新的新手教程」，
	## 重做期间入口不开（老教程退出再启动会崩，没复现出来，索性不让玩家碰到）。
	## 灰掉走的是菜单自带那条路（`.mi.dim` 画灰 + 不吃鼠标 + 键盘跳过 + `_activate` 直接返回），
	## **新版做好随全量发版一起把这里改回 true**，别的一行不用动。
...
func _item_enabled(i: int) -> bool:
	...
	##
	## 2026-09-10 起这一项由 `CWMatch.REPLAY_ON` 管（Kevin 拍板临时下架）。
	## **灰着而不是抽掉**：八项的位置是场景里钉死的，抽掉一项会在中间留个洞；
	## 灰掉还顺带告诉玩家「这功能在，只是这一版先不开」。见 docs/临时下架清单.md
	if ITEMS[i]["node"] == "Replay":
		return CWMatch.REPLAY_ON
	return true
