...
##
## **卡面只放卡名 + 类别，效果文只在悬停详情框里出现 —— 这是设计，不是没做完**
## （团队 2026-09-01 确认）。别再有人「顺手把效果印到卡面上」：
## 卡只有 72×112，10px 点阵字一行放得下 6 个字，而效果正文最长一百多字；
## 真要印上去就得先重新定卡的尺寸和手牌区的排布，那是另一件事。
## 效果原文住在 `CWCardData.effect_of()`，由 `CWCardInfo` 悬停时展示。
##
