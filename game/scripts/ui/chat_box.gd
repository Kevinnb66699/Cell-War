...
## ## 两种样子
##
## **关着**（大多数时候）：左下角一条常驻提示「聊天　Enter」，有未读时跟一个数字；
## 提示上方浮出**最近两条**消息，几秒后淡掉。不开框也看得到刚才说了什么。
##
## **开着**：标题栏（可拖）+ 全体/己方切换 + 消息列 + 输入行。
##
## ## 三条不做的
##
...
## · **浮出条不跟着框走** —— 框拖到哪儿是玩家的事，而框关着的时候「哪儿」不存在。
##   所以浮出条固定在左下（= 框的默认位置）。
##
## ## 阵营用颜色分，不写字
##
...
const RECT := Rect2(16, 300, 300, 176)   ## 默认位置 = 左下（同浮出条），可拖
const HINT_AT := Vector2(16, 486)        ## 常驻提示：框的正下方
const FLOAT_N := 2                       ## 关着时浮出几条（一条看不全一来一回，三条起压棋盘）
const FLOAT_HOLD := 6.0                  ## 浮出条停多久开始淡
const FLOAT_FADE := 1.2
const ROW_H := 18.0
const MAX_ROWS := 6                      ## 框里最多显示几条
const PAD := 10.0

var _lines: Array = []                   ## 收到的消息，新的在后
var _open := false
var _unread := 0
var _hold := 0.0                         ## 浮出条还剩多久开始淡
var _drag := false
...
func _process(delta: float) -> void:
	if _hold <= 0.0:
		return
	_hold -= delta
	var a: float = clampf(_hold / FLOAT_FADE, 0.0, 1.0)
	for l in _floats:
		l.modulate.a = a
	if _hold <= 0.0:
		_repaint()
...
func _build() -> void:
	...
	var title := CWStyle.label("聊天", CWStyle.SIZE_LABEL, CWStyle.TEXT_HI)
	...
	## 关着时的两样：浮出条（在提示上方往上排）与常驻提示
	for i in FLOAT_N:
		var l := CWStyle.label("", CWStyle.SIZE_LABEL, CWStyle.TEXT_HI)
		l.position = Vector2(HINT_AT.x, HINT_AT.y - (FLOAT_N - i) * ROW_H)
		l.size = Vector2(RECT.size.x, ROW_H)
		l.clip_text = true
		l.text_overrun_behavior = TextServer.OVERRUN_TRIM_ELLIPSIS
		l.mouse_filter = Control.MOUSE_FILTER_IGNORE
		add_child(l)
...
func _on_bar_input(e: InputEvent) -> void:
	if e is InputEventMouseButton and e.button_index == MOUSE_BUTTON_LEFT:
		_drag = e.pressed
		_drag_from = _panel.global_position - e.global_position
	elif e is InputEventMouseMotion and _drag:
		## 拖出屏幕就再也抓不回来了，所以钳在可视范围内
		var to: Vector2 = e.global_position + _drag_from
		_panel.position = Vector2(
			clampf(to.x, 0.0, CWView.screen_size().x - RECT.size.x),
			clampf(to.y, 0.0, CWView.screen_size().y - RECT.size.y))
...
func _repaint() -> void:
	...
	## 关着时：浮出最近两条 + 提示带未读数
	var show_float := not _open and _hold > 0.0
	for i in FLOAT_N:
		var idx: int = _lines.size() - FLOAT_N + i
		var l: Label = _floats[i]
		l.visible = show_float and idx >= 0
		if l.visible:
			l.text = line_text(_lines[idx])
			l.add_theme_color_override("font_color", line_color(_lines[idx]))
			l.modulate.a = 1.0
	_hint.visible = not _open
	_hint.text = "聊天　Enter" if _unread <= 0 else "聊天　Enter　%d" % _unread
	_hint.add_theme_color_override("font_color",
		CWStyle.TEXT_HI if _unread > 0 else CWStyle.TEXT_DIM)
