...
## 也往 active 里塞条目，复用全部挂接点** —— 这是先做世界事件的原因。已经住进来的
## 有【基质稳定】【TGF-β释放】【TNF-α局部炎症】。要区分「这条是世界事件还是卡牌挂的」
## 用 is_world_event()，别拿 DURATION 表当判据（那张表只列世界事件，审查问题 3）。
## **当前排期下两个世界事件不会同时在场**（2026-08-30 核算，纠正早先相反的说法；
## 注意这句只管世界事件——加上卡牌条目，容器里同时有三条是正常的）：
## 触发回合 3/6/10/15/20/25/30 最短间隔 3 回合；普通持续事件活 2 回合，
## 最长的是被【双重触发】拉到 4 回合的开关类，而它最早只能从第 6 回合起手
## （第 3 回合得先抽到双重触发），活 6~9 回合，第 10 回合的新事件正好接上。
## **6→10 这段是刚好卡满、零余量**——改事件表或加长持续时间就会重叠。
## 容器是列表、各结算点独立查询，真重叠了也撑得住（#26 的卡牌修饰同理）。
##
...
## 16 个事件，按 PRD「世界事件」节的出场顺序。近期随 PRD 正本改动三处：
## ·【细胞毒】改名【抗原引导】（2026-09-07）—— 原名与 T 细胞技能【细胞毒素】只差一字，读日志时分不清；
## ·【固化加速】删除（2026-09-07）—— 固化阈值 2026-09-01 降到 2.0 之后它与常规固化完全同义，是死机制；
## ·【免疫抑制因子】删除（2026-09-08，Kevin）—— 一个事件同时压净化价、压记忆、扣癌方能量、逼免疫弃牌，
##   四件事挤在一起。删掉之后**场上不再有任何世界事件抬高免疫的迁移费**（见 CWCost 的世界事件段），
##   也不再有世界事件伤害癌细胞（【囊性护甲】那条测试随之改成守契约，不再有活路径）。
## ·【抗原丢失】删除（2026-09-08 云端修订版）—— 它是**唯一**会整体免疫掉一次伤害事件的世界事件。
##   删掉之后伤害管线的「替代/免疫」层（设计 §5.2）**再没有活的触发者**：
##   `CWDamage._immune_to()` 保留成契约（那一层是设计里的一环，将来的卡牌会住进去），
##   但它现在恒为 false，相关测试随之改成守契约而不是走活路径。
const EVENTS: Array = [
...
## 每个事件的一句话效果（按 PRD「世界事件」节缩写，不含「持续 N 回合」—— 持续期由调用方按 left 拼）。
## 给两处用：抽到时棋盘上方的通报（trigger → CWGame.notice），右侧竖条事件行的悬浮详情（CWMatchPanel）。
## 2026-09-02 Kevin：世界事件此前只有日志里看得到。改 PRD 文案时这里要跟着改（prd_crosscheck 不核对它）。
const BLURB := {
	"营养输送": "每个细胞首次通过血管时 +2.0 能量并抽 1 张",
	"异常增殖": "组织增生概率翻倍",
	"代谢加速": "进入代谢核心获得的能量翻倍",
	"紊乱": "所有细胞随机传送到己方组织，回合末返回原位",
	"抗原引导": "免疫攻击不会失败（5/6 成功、1/6 大成功）",
	"免疫伪装": "免疫攻击不会大成功；癌细胞移动 +0.2",
	"抗原变异": "免疫攻击失败则被打的癌细胞抽 1 张，大成功则免疫抽 1 张",
	"抗原暴露": "每次获得抗原记忆额外 +1",
	"增殖抑制": "组织无法增生；免疫全体 -0.5，癌细胞各弃 1 张",
	"营养缺乏": "代谢核心与骨髓清空、本回合不产出",
	"双重触发": "下一个世界事件触发两次（若可能）",
	"细胞应激": "打出卡牌要付 0.5",
	"基质阻隔": "癌细胞移动能量花费翻倍",
	"迁移激活": "每个免疫细胞每回合首次移动免费",
	"信号放大": "卡牌效果的能量增减翻倍",
}

## 持续 2 回合的事件；其余为「本回合」（回合末撤销）
const DURATION := {
	"营养输送": true, "异常增殖": true, "代谢加速": true, "抗原引导": true,
...
func on_round_start() -> void:
	for e in game.events["active"]:
		...
		elif is_world_event(e) and not DURATION.has(e["name"]):
...
## 这个条目是**世界事件**，还是卡牌挂上来的全局修饰？两者共用 events["active"]
## （#26 的框架承诺），但只有世界事件有「本回合类 / 持续 2 回合」的分类。
##
## 少了这一问，【TGF-β释放】（卡牌挂的、left=2、自然不在 DURATION 表里）会掉进
## 上面那个分支，每次都喊一句根本没发生过的「【双重触发】…第二回合再次生效」
## ——`_resolve` 对它是空操作，所以没有数值影响，但日志是玩家理解结算的唯一窗口
## （2026-08-30 审查问题 3）。
func is_world_event(entry: Dictionary) -> bool:
	return entry["name"] in EVENTS
