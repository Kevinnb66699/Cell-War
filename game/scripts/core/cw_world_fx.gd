...
## 17 个事件，按 PRD「世界事件」节的出场顺序。2026-09-07 随 PRD 正本改动两处：
## ·【细胞毒】改名【抗原引导】—— 原名与 T 细胞技能【细胞毒素】只差一字，读日志时分不清；
## ·【固化加速】删除 —— 固化阈值 2026-09-01 降到 2.0 之后它与常规固化完全同义，是死机制。
const EVENTS: Array = [
	"营养输送", "异常增殖", "代谢加速", "紊乱", "抗原引导", "免疫伪装",
	"抗原变异", "抗原丢失", "抗原暴露", "增殖抑制", "免疫抑制因子",
...
const BLURB := {
	...
	"免疫抑制因子": "净化多付 0.2 且不得记忆；癌细胞全体 -0.5，免疫各弃 1 张",
