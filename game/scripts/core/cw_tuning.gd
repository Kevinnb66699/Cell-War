...
## 【E-无氧呼吸】在**世界回合 E 阶段**统一结算（Kevin 2026-09-06 改回，默认 false）。
## true = 09-05 那版「各癌细胞自己的行动回合末」各算各的（`eturn=1` 对照档）——当时改过去的理由留在下面：
...
## 09-05 晚只拨 eturn 的 A/B（c=2.0）：E 阶段档四人 41→50、六人 41→29，每局少死约半个癌细胞。
...
## **与 PRD 一致**（2026-09-04 起）：新 PRD 给【转移】加了每世界回合次数上限，
## 当天下午又从 1 次改成 **2 次**（「借道前进」那一版 PRD）。
## ⚠ 09-03 量过这根杠杆（§10.6）：限次 1 **不但没削弱黑+小，反而把它从 62% 抬到 79%**
## （AI 少跳之后能量全花在定殖上）。Kevin 定「以最新版 PRD 为标准、重新测」，
## Kevin 定「以最新版 PRD 为标准、重新测」，重测结果见平衡方案对比 §十一。
## 现值 **2**，与 PRD 现行的「每世界回合至多使用两次」一致（这句注释以前还写着「置 1」，
## 是 09-04 上午那一版留下的，2026-09-09 核对时改正）。0 = 不限。
var metastasis_max_per_round := 2
