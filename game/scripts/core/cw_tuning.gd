...
## ⚠ **引擎落后于 PRD**：PRD【迁移】写的是「1/3概率失败，不造成伤害，**自身-0.5能量**」，
## 也就是这里应该是 5。默认值 0 是 61 格旧规则留下的——旧规则原文确实没有那半句。
## **改之前先看 [PRD差异对照 §8.1](../../../docs/PRD差异对照.md)**：改成 5 会给癌方
## 第二条稳定伤害途径，而现行平衡（口径 #82）是在「攻击不自损」的前提下标定的，
## 改完必须重跑平衡。旧注释曾断言「规则原文为 0…免疫细胞不可能死亡（说明 #23）」——
## 那句是假话，而且正因为它写得斩钉截铁，2026-08-28 的全量比对读到这里就跳过去了。
