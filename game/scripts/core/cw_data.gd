...
# ---- 特殊组织（2026-08-27 团队定案「方案 A」，轴坐标，中心 (0,0)）----
# 布局：半径 4 的那一环的 6 个「角」上，代谢核心与骨髓**交替**排布（各 3 个）；
#      骨髓再在外缘的「边中格」补 3 个；左右两个最远角 (±6,0) 一对血管。
#
# 为什么骨髓补在边中格而不是边角：要保住核心那圈的 **3 重旋转对称**（转 120° 完全重合），
# 新增点必须是「转 120° 后落回自身」的一组。外缘满足条件的只有两类——6 个角和 6 个边中格，
# 而 6 个角分成的两组轨道**各被一个血管占了**，所以只剩边中格可选。
# 结果是每个 120° 扇区正好分到 1 核心 + 1 内圈骨髓 + 1 外圈骨髓。
#
# 中央格原本也是骨髓，2026-08-27 移除——它与「初始癌组织必须包含中央格 + 癌组织不得与
# 特殊组织重合」这两条规则直接冲突（说明 #34），且会让癌方从第 0 回合白拿一个骨髓。
#
# 可视化参考：docs/地图布局_127格.svg（坐标标在每格上，可与美术图逐格核对）
const CORES: Array[Vector2i] = [Vector2i(0, -4), Vector2i(4, 0), Vector2i(-4, 4)]
const MARROWS: Array[Vector2i] = [
	Vector2i(4, -4), Vector2i(-4, 0), Vector2i(0, 4),      # 内圈：与核心交替
