...
##
## ⚠ **2 人局 PRD 仍未定**（只写了四人与六人两档），沿用 2.8。它不在平衡目标内。
...
##
## ⚠ **二人局有意留在缺省档（10/20/30）**。按上面那套道理它只有 1 个免疫、该比四人局更低才对，
## 但 PRD 只定了四人与六人两档，替它挑一个数就是我在定平衡值 —— 那是团队的事。
## 维持现状 + 记在这儿，等团队给数。（`{ 2: [...] }` 加进表里就生效，别的都不用动。）
