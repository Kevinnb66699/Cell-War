...
##   能量损失 = max{0, ((x − 10) ÷ 2) ^ 1.18}     x = 该癌细胞当前能量（显示单位）
##
## 三个数各一个旋钮（balance_scan 要拨）。**它和上面的 ENERGY_CAP_PER_ROUND、
## 平衡候选③ `cancer_upkeep_pct` 职能重叠，但曲线完全不同**：硬顶对阈值以下毫无作用、
## 抽税对每一档等比例，而这条是**凸的** —— 越囤扣得越狠。PRD 给了这一条之后，
## 那两个候选大概率不再需要；但它们现在**都默认关着**（都是 0），所以这次不动它们，
## 等平衡扫过再定去留。
##
## 实测这条曲线的三个拐点（`((e − 100) ÷ 20)^1.18`，e 为十分能量；见 CWWorld.overload_loss）：
##   · **实际门槛是 10.2 不是 10.0** —— 10.1 的损失舍入后为 0.0
##   · **净留极大在 42.2**（净留 15.7）—— 再往上囤，留下的反而变少，这是这条规则的真正形状
##   · **148.4 处损失追平能量、148.8 起严格反超** —— 所以必须钳，否则能量会被扣成负数
