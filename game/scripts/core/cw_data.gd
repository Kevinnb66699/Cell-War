...
# 团队 2026-09-04 定案：【E-无氧呼吸】改成 **c × √(连通块癌格子数) ÷ 连通块癌细胞数**。
# c 单位十分能量；0 = 退回旧的线性求和（对照档用）。
# 沿革：2026-09-05 由 MC 扫描定 2.0（在「各癌细胞回合末结算」下调出）；**2026-09-06 Kevin 改 1.0**，
# 同日结算时机改回 E 阶段（CWTuning.anaerobic_on_turn_end 默认 false）。09-05 那套 41%/41% 的标定随之作废，新值等团队手打。
const ANAEROBIC_SQRT_COEF := 10
...
## **与 PRD 一致**（2026-09-04 起）。历史：2026-09-03 曾按定案② 把 X 级从 0.5 改成 0.7，
## 09-04 团队覆盖 PRD 正本时又写回「X 级迁移耗能均改为 0.5」，Kevin 定「以最新版 PRD 为标准、重新测」——
## 于是这里回到 PRD 值。旋钮 `CWTuning.immune_move_cancerous[3]`，balance_scan `mvx=7` 可扫回定案②。
const IMMUNE_MOVE_CANCEROUS := [10, 10, 7, 5] # 迁移→癌性，按等级 I/II/III/X
...
const AEROBIC_LEVEL_BASE := 20                # 十分能量：四人局/二人局的「+2.0」，也是表外人数的回退值
...
const AEROBIC_LEVEL_STEP := 5                 # 十分能量，公式里的「等级 × 0.5」
...
## 团队 2026-09-05 定：免疫细胞**踏进**黏液格时迁移费 +0.5。癌细胞不受影响。
const MUCUS_MOVE_SURCHARGE := 5          # 十分能量
...
const MACRO_HEAL_PURIFY := 3             # 巨噬【吞噬】：每次净化回多少（十分能量）
...
## 建立一次的花费（十分能量）、持续几个世界回合、免疫朝它走的折扣与癌方背它走的加价（百分数）。
## **同一时刻场上仅一个**（PRD 明文）。
const CHEMO_COST := 20
...
const SKILL_TEXT := {
	...
	"chemo": "【I-趋化源】：树突状细胞可消耗2能量在全局任意位置建立趋化源，效果持续2回合\n"
		+ "· 免疫细胞向靠近该格方向移动时，迁移费用减免30%\n"
		+ "· 癌细胞向远离该格的方向移动时，移动费用增加40%\n"
		+ "　· 是否靠近/远离取决于，迁移后距离该格的距离增加/降低\n"
		+ "· 同一时刻场上仅能存在一个趋化源",
