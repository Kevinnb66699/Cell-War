...
const IMMUNE_MOVE_CANCEROUS := [10, 8, 7, 7]  # 迁移→癌性，按等级 I/II/III/X
...
## 这一张是 **6 人局**的，也是没列人数时的缺省 —— 分档表见下面。
const LEVEL_MIN_MEMORY := [0, 10, 20, 60]
## 门槛**按人数分档**（Kevin 2026-09-09）：四人局 6 / 16 / 30，六人局 10 / 20 / 30。
##
## 为什么要分：记忆是**全阵营共用一个计数器**，而进账靠免疫细胞各自净化。
## 四人局只有 2 个免疫、六人局有 3 个，同样的门槛下四人局要多花约一半的回合才升得上去 ——
## 分档就是把「升级要几个回合」拉回同一档，而不是让人少的一方顺带被罚。
## X 级两档相同（30）：那一档 PRD 从来只写一个数，团队也没提要分。
##
## 2 人局沿用缺省档（10/20/30）—— **这不是待办**。按上面那套道理它只有 1 个免疫、
## 门槛该更低，但 2 人局在平衡目标之外（Kevin 2026-08-31 与 09-09 两次确认，
## 见《PRD差异对照》§7.6），不必替它挑数。真要定的话 `{ 2: [...] }` 加进表里就生效。
## issue #13（2026-09-10）把 **X 级**的门槛抬高了一大截：
## 四人 30 → **50**、六人 30 → **60**（III 级的区间也因此变成 4 人 16~49 / 6 人 20~59）。
## II / III 两档没动。X 级是「记忆改名效应记忆并从零重数 + 解锁效应应答与 X 级卡池」，
## 原来 30 的门槛让它来得太早，整局后半段几乎都在 X 级上。
const LEVEL_MIN_MEMORY_BY_PLAYERS := { 4: [0, 6, 16, 50] }
...
const SOLIDIFY_THRESHOLD_BY_STAGE: Array[int] = [30, 30, 20]   # 计数达 3.0 → 固化癌组织；III 期 2.0
