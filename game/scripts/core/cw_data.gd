...
# ---- 场景事件（PRD 场景事件）----
# 【E-微环境压迫】：损失 = max(0, 相邻癌组织 + 相邻固化癌组织 × 2 − 相邻健康组织) × 0.5。
...
##
## 上下限由 0 ~ 2.0 变成 **0 ~ 6.0**（六面固化）；同为六面癌组织时 2.0 → 3.0。
## ⚠ 线上 PRD 的公式**漏了末尾的 × 0.5**（= PRESSURE_PER_ADJ），
## 照那条字面实现会做成两倍。Kevin 2026-09-08 当面确认 ×0.5 保留，
## 见 docs/PRD改动_2026-09-08_压迫公式.diff。
...
const PRESSURE_PER_ADJ := 5              # 加权和的每 1 点 = 0.5 能量
