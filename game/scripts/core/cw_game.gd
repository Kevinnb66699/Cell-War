...
var sim_quiet := false
## 判不判那两条「立刻赢」（【E-免疫胜利】/【E-癌症胜利】）。**教程 1–15 关关掉**（CWGuideDirector.assemble）：
## 那些局面是教学摆拍 —— 癌方只有一枚死亡占位、场上常常一块固化都没有，E 阶段第 10 步的【E-免疫胜利】
## 一判就成立，第一章刚过、第二关刚开就弹结算屏（Kevin 2026-09-12 截图）。教程什么时候结束由引导剧本说了算
## （CWGuide.on_chapter_done），不由胜负条件说了算；毕业战（第 16 关）是正式四人局，照常判。
## **回合上限那条终局不受它管**（见 CWWorld.e_phase 第 10 步）：每一局都得有个终点。
## 和 sim_quiet 一样是**运行期开关**：不进快照、不进协议、不是旋钮，正式对局与联机一行都不受影响。
var win_checks := true
