...
static func make_tile(c: Vector2i) -> Dictionary:
	return {
		...
		"cards": 0,      # 骨髓存储的卡牌数
		"prod": 0,       # 特殊组织产出周期计数器
		## 上一次在这一格发动【细胞毒素】是第几个世界回合；0 = 从没有过。
		## 「每世界回合同一格子上仅能发动一次」（2026-09-10 issue #10 的新规则）——
		## 记回合号而不是布尔值，就不必在 S 阶段挨格清一遍。
		"toxin_round": 0,
	}
...
static func make_cell(id: int, pid: int, faction: int, pos: Vector2i,
	...
	return {
		...
		"effector_used": false,    # 【效应应答】每个细胞每局仅 1 次；**死亡复活后仍保留**
		...
		"revives": 0,              # 本细胞已结算过几次【复活】；只记账（issue #68 回调后不再进死亡惩罚 X）
