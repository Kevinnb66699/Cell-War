...
func poll_host(now: int) -> void:
	if _udp == null or _beacon.is_empty():
		return
...
func poll_listen(now: int) -> bool:
	...
	while _udp.get_available_packet_count() > 0:
		...
		var e := parse_beacon(bytes, _udp.get_packet_ip(), now)
		if not e.is_empty():
