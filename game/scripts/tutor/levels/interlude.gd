...
## 分镜 8（PRD:431）：离玩家最近的那只免疫喊话。
## `say.who` 写成 `"seat:<n>"`（PRD 的「周围免疫」在表现层就是它头上那只气泡）
func alarm(ctx) -> void:
	var seat := _nearest_immune(ctx)
	if seat < 0:
		await ctx.fail("分镜 8：盘上一只活着的免疫细胞都没有，没人喊得出这一句")
		return
	ctx.log("分镜 8：离玩家最近的免疫是席位 %d" % seat)
	await ctx.beat({ "do": "say", "prd": 431, "who": "seat:%d" % seat,
		"lines": ["发现新的敌人，继续清除——"], "auto": true, "beats": 2 })
