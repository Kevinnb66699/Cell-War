...
## · B：第 1 回合走到 (-4,0)（0.5）发三发抗体（3.0），第 2 回合踩着一环走进死巨噬那格 (-2,0)（两步各 0.8，
##   顺手把 (-3,0)(-2,0) 净化掉），结算时五面是癌组织（1.2）剩 0.9；第 3 回合玩家把一环重新踩满（(-3,0) 再【定殖】），
##   B 打玩家一下（0.8）剩 0.1、掷骰无效再被【反弹】归零 —— 不然结算 1.5 压死。
