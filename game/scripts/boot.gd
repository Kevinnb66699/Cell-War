...
## 预算抬到 10 了，这口跟着抬才不会出现「元数据还能等、包反而先超时」。
const NET_TIMEOUT := 10.0
...
## **2026-09-13 Kevin 改成 10 秒**：查更新等满 10 秒还没结果才自己进游戏。
const CHECK_BUDGET := 10.0
## 等了这么久还没查完，就把「跳过」露出来。**2026-09-13 Kevin 定 6 秒**（原来 2 秒）——
## 网络正常时查更新一两秒就完，按钮亮得太早反而像「出问题了」。
##
## **必须早于自动兜底**（`CHECK_BUDGET`），否则按钮刚亮游戏就自己进去了；
## 现在是 6 秒亮、10 秒兜底，中间留了 4 秒给玩家按。
## 护栏 `t_hot_patch` 钉着 SKIP_AFTER < CHECK_BUDGET 这条关系。
const SKIP_AFTER := 6.0
