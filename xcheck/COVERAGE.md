...
| `quote_path` | OK | required | 12 | `quote_path` ↔ `RulePolicies.QuotePath` | 批 1 落地，第一个 `tree`：投影键表 `{ok,stop,total,left,gained,steps:[{to,cost,mid,afford,blocked,gain}]}`（**不收 `legal`**：GD 不看余额、C# 恒等于 afford；`blocked` 收 0/1 不收文案）。空档：F2 额度预演三条、F12 敌方占位那一步的 cost（见三） |
...
| **F2 · C# `QuotePath` 不预演费用额度** —— GD 每走通一步 `burn_allowances`（issue #35），C# 一步都不烧 `fx_turn`：装【组织驻留】走三步 GD 报 0/0/5、C# 报 0/0/0。`t_plan_allowance` 9576 / 9580 / 9590 三条用例已写好（只要 `equipped` + `fx_turn`），`quote_path` 不设 `allowance` 档 | 批 1 落地后紧接着合（规则结果差，按口径一补充口径「差的是规则结果才合」） | 批 1 P1 / P2 各自取证 |
| **F12 · `quote_path` 敌方占位那一步的 `cost`** —— GD 在 occupied 分支根本不报价（0），C# 无条件先 `QuoteMove`（给的是攻击价）。`blocked` 档只用同阵营占位、任何 `quote_path` 路径不经过敌方占位格 | 与 F2 一起（2 行：`QuoteMove` 挪进 else） | 批 1 P1 / P2 |
...
| **机件三条（批 1 登记、未修）**：① F13 delta 的无语义键数组叶子 C# `Subset.Text` 序列化成一行再比（键序敏感）、GD `cw_case_diff.gd:compare` 结构比 —— 手写 `changed` 里的对象要按 C# 键序（`enter_tile/healthy/colonize_clears_necrosis` 就是这么写的），修法 = `Subset.Text` 序列化前按键名排序；② F15 契约门② 对 `cases: deferred` 不设断言，deferred 行偷偷带用例抓不到；③ `erosion` 的 `fresh` 参数 GD 读 JSON 数组、C# `Args.Positions` 读分号串（`L0Case.Args` 是 `Dictionary<string,string>`，数组连反序列化都过不去）—— **批 3 开工前先合**（推荐 GD 改读分号串） | 机件，随批 3 前 | 批 1 P1 / P2 |
