...
| `const` | OK | required | 40 | 两侧各一张 42 键表（`l0_runner.gd:_build_consts` ↔ `Probes.cs:ConstTable`，19 真 + 21 抛「无对应物」） | 批 0（09-19）：`batch0/const_data.json` 28 + `const_card.json` 4；三档 cw_data 28 / cw_card_data 4 / 静态函数 20 批 5b（09-19）+2 表项（随世界事件删除作废，2026-09-19 两项常量已不存在）：`CWWorldFx.EVENTS`（15 名事件表 → tree，钉两侧名表逐字相同 —— 此前没有任何东西钉它）/ `CWWorldFx.is_world_event`（bool → 1/0；名字收口）；`batch5b/const_fx.json` 3 + `const_event_rounds.json` 5（`is_world_event_round` 3/6/10/14 触发 + 1 不触发） **2026-09-19**：两侧常量表 42 → **39 键**（删 `CWData.is_world_event_round` / `CWWorldFx.EVENTS` / `CWWorldFx.is_world_event`），`cw_world_fx` 那一档 boundaries 同删。 |
