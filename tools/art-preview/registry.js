...
import {cardItems} from './card-catalog.js';
import {cardEffects} from './card-effects.js';

export const items=[...originalItems,...immuneItems,...cancerItems,...commonItems].filter(item=>!removedIds.has(item.id)).map(revise).concat(extraItems,cardItems);
export const effects={hunt,neutralize,excalibur,chain,rupture,...immuneEffects,...cancerEffects,...commonEffects,...revisedEffects,...cardEffects};
export function matches(item,category,query) {
  const group=category==='all'||(category==='cards'&&item.id.startsWith('card_'))||(category==='original'&&(originalItems.some(original=>original.id===item.id)||item.id==='necrosis'))
