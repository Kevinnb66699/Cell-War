...
## 点阵字库里没有、但团队写 PRD 时会自然写出来的字符 —— 生成时替换掉。
##
## 为什么放在这儿而不是每次去改 PRD：「≤」这类符号 2026-08-29 / 09-09（两次）
## 一共让卡面上过三回豆腐块，每次的处理都是「请团队把 PRD 里的 ≤ 改成文字」——
## 而下一次团队照样会写 ≤，因为在规则文档里它本来就更精确。
## 与其反复要求人去迁就字库，不如让生成器负责这一步：
## **PRD 保持精确写法，上屏的那份自己转成字库有的字。**
##
## ⚠ 只替换**会上屏的卡面文案**。PRD 正文里的 ≤（通用规则、负担性判定…）不归这里管，
## 那些不进游戏。护栏 `t_font_coverage` 仍然拦得住这里没列到的其它缺字。
FONT_SUBS = {
    "≤": "或更低",     # 「能量降至≤0」→「能量降至0或更低」：跟在数字后面读不通，见下面的特判
    "≥": "或更高",
...
def font_safe(s):
    ...
    「≤0」不能直译成「或更低0」—— 中文里这个比较符是**前置**的，
    转成文字要挪到数字后面。所以先处理「≤<数字>」这种最常见的形式，
    剩下的孤立符号再按 FONT_SUBS 逐字替换。
    """
    s = re.sub(r"≤\s*(-?\d+(?:\.\d+)?)", r"\1或更低", s)
    s = re.sub(r"≥\s*(-?\d+(?:\.\d+)?)", r"\1或更高", s)
    for k, v in FONT_SUBS.items():
        s = s.replace(k, v)
    return s
...
A("\tif round_no <= 5:")
A("\t\treturn 0")
A("\treturn 1 if round_no <= 10 else 2")
A("")
A("")
