...
**这把尺只有一个方向**：`consts[k]` 的值只拿来打印，**从不参与比对**（main 里比的是
`MAP[k]` 那几句话在不在 PRD 里）。所以它能发现「PRD 改了、引擎没跟」，
**发现不了「引擎单方面把数改了」**——把 `PROLIFERATE_BASE_BY_STAGE` 改回 `[30,35,40]`
而不动 MAP，本脚本照样全绿。引擎改数只有无头测试挡得住，别拿这条当护栏。
...
## PRD 在仓库外（2026-09-11 起叫这个名）：正常是「仓库的上一级」，
## 但 git worktree 的上一级是 `.claude/worktrees/` —— 找不到就退到写死的那份（同 gen_card_data.py）。
PRD = os.path.join(os.path.dirname(REPO), "Cell_War_玩法PRD.md")
if not os.path.exists(PRD):
    PRD = "D:/Projects/SpringSense/2026-2027/Cell War/Cell_War_玩法PRD.md"
...
MAP = {
    ...
    ## 2026-09-14（issue #43）公式外面多了个人数系数 k：`max{2, ktimesfrac{…}}`，
    ## 锚点跟着从 `max{2,frac{` 改成 `max{2,ktimesfrac{`（六人局那句同时掉了个冒号）。
    "ANAEROBIC_BLOCK_COEF_BY_PLAYERS": ["四人局$能量=max{2,ktimesfrac{连通块癌组织个数^{0.3}times2.0",
                                        "六人局$能量=max{2,ktimesfrac{连通块癌组织个数^{0.3}times2.8"],
    ## 人数系数（2026-09-14，issue #43）：整句钉死 —— 三档值和「按块内癌细胞数取」这条口径都在这一句里
    "ANAEROBIC_CELLS_K":      "k为人数系数，癌性组织连通块内存活1/2/3个癌细胞时，k为80%/100%/120%",
    "ANAEROBIC_SOLID_BONUS":  "+连通块固化癌组织个数times0.5",
    ## 【S-过载】（PRD 2026-09-15 新增）：能量损失 = max{0, ((x−10)÷2)^1.18}。
    ## 三个数各锚一段，**每段都把自己那个数括进去** —— PRD 一改那个数，对应这一条就会失配。
    ## 门槛锚 `{x-10}` 而不是光锚 `10`：单独一个 10 在 1700 行里到处都是，锚不住。
    ...
    "LEVEL_MIN_MEMORY":       ["6人10-29抗原记忆", "6人30-119抗原记忆", "6人≥120抗原记忆"],
    # ---- 卡牌规则 ----
    ...
    # ---- 固化 / 场景事件 ----
    ## 环境恶化（2026-09-11）：三档表，I 期钉规则原句、II 期钉恶化效果那句。
    ## **III 期只有两条锚点**（2026-09-19 issue #64）：PRD 的 III 恶化效果里不再单列固化门槛，
    ## 按 Kevin 拍板沿 II 期的 2 —— 没有第三句可钉，硬编一条反而会把「PRD 又加回来了」盖住。
    "SOLIDIFY_THRESHOLD_BY_STAGE": ["计数到达3时**癌组织**转为**固化癌组织**",
                                   "【固化】所需的固化计数降低为2"],
    "SOLIDIFY_STEP":          "【E-固化】：癌细胞停留的**癌组织**的固化计数+1",
    ## `SOLIDIFY_DECAY` 2026-09-19 issue #64 随「计数不再递减」整条删除（PRD 的 E 阶段列表同日删掉那一步）。
    ...
    ## **2026-09-19 issue #64**：II 期 4%→5%、III 期 5%→6%（每固化那一半 1% / 1.5% 不动）。
    "PROLIFERATE_BASE_BY_STAGE": ["r_{增生概率}=3%+0.5%times所有相邻癌性组织联通块中固化癌组织数",
                                  "r_{增生概率}=5%+1%times所有相邻癌性组织联通块中固化癌组织数",
                                  "r_{增生概率}=6%+1.5%times所有相邻癌性组织联通块中固化癌组织数"],
    # ---- 特殊组织 ----
    ...
    "LEVEL_MIN_MEMORY_BY_PLAYERS": ["4人0~9抗原记忆", "4人10~19抗原记忆", "4人20~99抗原记忆", "4人≥100抗原记忆"],
    ...
    "PROLIFERATE_SOLID_BY_STAGE": ["3%+0.5%times所有相邻癌性组织联通块中固化癌组织数",
                                   "5%+1%times所有相邻癌性组织联通块中固化癌组织数",
                                   "6%+1.5%times所有相邻癌性组织联通块中固化癌组织数"],
    ## ⚠ 第一条锚点末尾**不带右括号**：PRD 那行的 LaTeX 是 `…-相邻健康组织})`，右花括号写在括号里面。
    "PRESSURE_MUL_BY_STAGE":  ["frac{1}{4}times(相邻癌组织+相邻固化癌组织times2-相邻健康组织",
                               "【E-微环境压迫】能量损失为1.5倍", "【E-微环境压迫】能量损失为2倍"],
    ## 【E-侵蚀】一次转几格：四条锚点 = 两档 × 两个概率分支（2/3 与 1/3 都写在 PRD 原句里）。
    ## I / II 期共用 (2,3) 那两句（PRD「场景事件」一节的原文就是 II 恶化那两句）。
    "EROSION_TILES_BY_STAGE": ["2/3概率2个健康组织被【侵蚀】", "1/3概率3个健康组织被【侵蚀】",
                               "2/3概率3个健康组织被【侵蚀】", "1/3概率5个健康组织被【侵蚀】"],
    ## 【E-无氧呼吸】的环境恶化增益（issue #56，2026-09-19 新增）：两档各钉一句。
    ## **2026-09-19 issue #64**：II 期 20% → 30%。
    "ANAEROBIC_STAGE_MUL_BY_STAGE": ["【无氧呼吸】在基础数值上增益20%", "【无氧呼吸】在基础数值上增益30%"],   ## issue #66：II 20% / III 30%
    ## 四张**癌种分期表**：2026-09-19 修好扫描器后才第一次进核对，各钉卡面那一句（三个数写在同一句里）。
    "GLUT1_BONUS":            "自身每次结算【无氧呼吸】时额外获得0.5/0.8/1能量",
    "RAS_HEAL":               "恢复0.3/0.5/0.7能量",
    "BCL2_ENERGY":            "能量改为0.5/0.8/1",
    "STEMNESS_ENERGY":        "复活后的初始能量由2提高至3/4/5",
    ## **2026-09-19 issue #64**：II 期 一格 → 三格、III 期 三格 → 五格。
    "ROOTED_BY_STAGE":        ["相邻最多一格**癌组织**的固化计数+1", "相邻最多二格**癌组织**的固化计数+1"],   ## issue #66：1 / 2 格
...
INTENTIONAL = {
    ...
    ## 2026-09-19 扫描器认了一行写完的 Vector2i 数组之后新进来的两张坐标表：
    ## PRD 只写「代谢核心（3个）」「血管（2个）」，**具体落在哪一格是电子版自己定的**（同 BOARD_RADIUS）。
    "CORES":               "PRD 只给个数（3 个代谢核心），坐标是电子版自己定的",
    "VESSELS":             "PRD 只给个数（2 条血管），坐标是电子版自己定的",
}
...
def main():
    ...
    for line in io.open(DATA, encoding="utf-8"):
        ...
        ## ⚠ 2026-09-19（issue #56 复核时发现）：这条此前写的是 `:= *`，而**带类型标注的常量在 GDScript 里
        ## 只能写 `: Array[int] = [...]`（没有 `:=`）** —— 于是七张 `_BY_STAGE` 分期表
        ## （固化门槛 / 增生两张 / 压迫倍率 / 根深蒂固 / 无氧分期增益）连同四张癌种分期表
        ## 整个没被扫过：MAP 里写着它们的锚点，却一条都没核过 —— PRD 把那几句改了也不会红。
        ## 与字典常量那次（2026-09-09）是同一类盲区：**「写法不同」= 悄悄跳过整整一类规则数值**。
        ## ⚠ 修好之后拿回来的也只是**那一个方向**：PRD 改了会红，引擎单方面改数照样绿（见模块头）。
        m = re.match(r"const (\w+)(?:: *Array\[int\])? *:?= *\[([\d, ]+)\]", t)
        if m:
            consts[m.group(1)] = [int(x) for x in m.group(2).split(",")]
        ## 一行写完的 Vector2i 数组（侵蚀格数表是唯一一张装规则数值的）。
        m = re.match(r"const (\w+): *Array\[Vector2i\] *= *\[(Vector2i\([\d, -]+\)[, ]*)+\]", t)
        if m:
            consts[m.group(1)] = t.split("=", 1)[1].split("#")[0].strip()
